...
async function main() {
  ...
  const vestingLockFee = BigInt(process.env.VESTING_LOCK_FEE || "0");
  const feeRecipient = process.env.VESTING_LOCK_FEE_RECIPIENT || deployer.address;

  console.log("vestingLockFee: ", vestingLockFee.toString(), "wei");
  console.log("feeRecipient:   ", feeRecipient);

  // @ts-ignore
  const Factory = await hre.ethers.getContractFactory("VestingTimeLock", deployer);

  const constructorArgs = [vestingLockFee, feeRecipient] as const;
  ...
  console.log("VestingTimeLock: ", contractAddress);
  ...
  if (shouldVerify) {
    ...
    try {
      await hre.run("verify:verify", {
        ...
        constructorArguments: [vestingLockFee.toString(), feeRecipient],
      });
      console.log("✓ VestingTimeLock verified");
    } catch (e: any) {
      if (e.message?.includes?.("Already Verified")) {
        ...
        console.log(
          `Verify manually: npx hardhat verify --network ${network.name} ${contractAddress} ${vestingLockFee} ${feeRecipient}`
        );
      }
    }
  } else {
    console.log(
      `\nTo verify: npx hardhat verify --network ${network.name} ${contractAddress} ${vestingLockFee} ${feeRecipient}`
    );
  }
}
